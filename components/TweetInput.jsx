...
function TweetInput() {
  ...
  async function sendTweet() {
    ...
    if(image){
      const imageRef = ref(storage, `tweetImage/${docRef.id}`);
  ...
  return (
    ...
    {!loading && ( <div className="w-full">
        ...
        <div className="flex justify-between border-t border-gray-700 pt-4">
          ...
          <button onClick={sendTweet} disabled={!text && !image} className="bg-[#1d9bf0] rounded-full px-4 py-1.5 disabled:opacity-50">
            Tweet
          </button>
        </div>
      </div>)}
